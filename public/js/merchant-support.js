...
async function createTicket(e) {
    ...
    const onBehalfOf = form.querySelector('[name="on_behalf_of"]').value;
    const category = form.querySelector('[name="category"]').value;
    const subject = form.querySelector('[name="subject"]').value.trim();
    const message = form.querySelector('[name="message"]').value.trim();

    if (!onBehalfOf || !category || !subject || !message) {
    ...
    const result = await post(platformLinks.api.merchant.support.create, {
...
async function addReply(ticketUid) {
    ...
    const result = await post(platformLinks.api.merchant.support.reply, {
...
function closeTicket(ticketUid) {
    SweetPrompt.confirm('Luk henvendelse?', 'Er du sikker på, at du vil lukke denne henvendelse?', {
        ...
        onConfirm: async () => {
            const result = await post(platformLinks.api.merchant.support.close, {
                ticket_uid: ticketUid
            });
...
function reopenTicket(ticketUid) {
    SweetPrompt.confirm('Genåbn henvendelse?', 'Er du sikker på, at du vil genåbne denne henvendelse?', {
        ...
        onConfirm: async () => {
            const result = await post(platformLinks.api.merchant.support.reopen, {
                ticket_uid: ticketUid
            });
