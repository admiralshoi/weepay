/**
 * Orders Table - Backend Pagination, Search, Filter, and Sort
 * Uses server-side pagination for scalability
 */
const OrdersPagination = (function() {
    // State
    let currentPage = 1;
    let perPage = 10;
    let searchTerm = '';
    ...
    function init() {
        ...
        $dateRangeClear = $('#orders-daterange-clear');
        $showing = $('#orders-showing');
        $total = $('#orders-total');
        $currentPageEl = $('#orders-current-page');
        $totalPages = $('#orders-total-pages');
        $pagination = $('#orders-pagination');
        $noResults = $('#orders-no-results');
        $paginationContainer = $('#orders-pagination-container');

        if (!$tbody.length || typeof ordersApiUrl === 'undefined') return;
    ...
    async function fetchOrders() {
        ...
        try {
            ...
            const result = await post(ordersApiUrl, params);
            ...
            <tr>
                <td colspan="8" class="text-center py-4">
                    <div class="flex-col-center flex-align-center">
                        ...
                        <button class="btn-v2 mute-btn mt-2" onclick="OrdersPagination.refresh()">Prøv igen</button>
                    </div>
                </td>
            </tr>
        `);
