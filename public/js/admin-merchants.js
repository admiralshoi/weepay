/**
 * Admin Merchants Table - Backend Pagination, Search, Filter, and Sort
 * Uses server-side pagination for scalability
 */
const AdminMerchantsPagination = (function() {
    // State
    let currentPage = 1;
    let perPage = 25;
    let searchTerm = '';
    ...
    function init() {
        ...
        $filterStatus = $('#merchants-filter-status');
        $sort = $('#merchants-sort');
        $perPage = $('#merchants-per-page');
        $showingStart = $('#merchants-showing-start');
        $showingEnd = $('#merchants-showing-end');
        $total = $('#merchants-total');
        $totalCount = $('#merchants-total-count');
        $pagination = $('#merchants-pagination');
        $noResults = $('#merchants-no-results');
        $paginationFooter = $('#merchants-pagination-footer');

        if (!$tbody.length || typeof adminMerchantsApiUrl === 'undefined') return;
    ...
    async function fetchMerchants() {
        ...
        try {
            const params = {
                ...
                role: '2', // Merchant access level
            };

            if (searchTerm) params.search = searchTerm;
            if (filterStatus) params.status = filterStatus;

            const result = await post(adminMerchantsApiUrl, params);
            ...
            <tr>
                <td colspan="4" class="text-center py-4">
                    <div class="flex-col-center flex-align-center">
                        ...
                        <button class="btn-v2 mute-btn mt-2" onclick="AdminMerchantsPagination.refresh()">Prøv igen</button>
                    </div>
                </td>
            </tr>
        `);
    ...
    function renderMerchants(users) {
        ...
        users.forEach(function(user) {
            html += renderMerchantRow(user);
        });

        $tbody.html(html);
    }

    function renderMerchantRow(user) {
                ...
                <td>
                    <div class="flex-row-start flex-align-center" style="gap: .75rem;">
                        <div class="square-40 bg-light-gray border-radius-50 flex-row-center-center">
                            <i class="mdi mdi-store font-20 color-success"></i>
                        </div>
    ...
    function renderPagination() {
        ...
        $pagination.find('a[data-page]').on('click', function(e) {
            ...
            if (page !== currentPage && !isLoading) {
                ...
                fetchMerchants();
                $('html, body').animate({
                    scrollTop: $('#merchants-table').offset().top - 100
                }, 200);
            }
        });
    }
