/**
 * Admin Organisations Table - Backend Pagination, Search, Filter, and Sort
 * Uses server-side pagination for scalability
 */
const AdminOrganisationsPagination = (function() {
    // State
    let currentPage = 1;
    let perPage = 25;
    let searchTerm = '';
    ...
    function init() {
        $tbody = $('#organisations-tbody');
        $table = $('#organisations-table');
        $search = $('#organisations-search');
        $filterStatus = $('#organisations-filter-status');
        $sort = $('#organisations-sort');
        $perPage = $('#organisations-per-page');
        $showingStart = $('#organisations-showing-start');
        $showingEnd = $('#organisations-showing-end');
        $total = $('#organisations-total');
        $totalCount = $('#organisations-total-count');
        $pagination = $('#organisations-pagination');
        $noResults = $('#organisations-no-results');
        $paginationFooter = $('#organisations-pagination-footer');

        if (!$tbody.length || typeof adminOrganisationsApiUrl === 'undefined') return;
    ...
    async function fetchOrganisations() {
        ...
        try {
            ...
            const result = await post(adminOrganisationsApiUrl, params);
            ...
            <tr>
                <td colspan="5" class="text-center py-4">
                    <div class="flex-col-center flex-align-center">
                        ...
                        <button class="btn-v2 mute-btn mt-2" onclick="AdminOrganisationsPagination.refresh()">Prøv igen</button>
                    </div>
                </td>
            </tr>
        `);
    ...
    function renderOrganisations(organisations) {
        ...
        organisations.forEach(function(org) {
            html += renderOrganisationRow(org);
        });

        $tbody.html(html);
    }

    function renderOrganisationRow(org) {
        const statusLabels = {
        ...
        const status = statusLabels[org.status] || { label: 'Ukendt', class: 'mute-box' };
        const truncatedUid = org.uid.substring(0, 10);
        ...
        return `
            <tr>
                <td>
                    <div class="flex-row-start flex-align-center" style="gap: .75rem;">
                        <div class="square-40 bg-light-gray border-radius-8px flex-row-center-center">
                            <i class="mdi mdi-domain font-20 color-blue"></i>
                        </div>
                        <div class="flex-col-start">
                            <a href="${adminOrgDetailUrl}${org.uid}" class="mb-0 font-14 font-weight-medium color-blue hover-underline">${escapeHtml(org.name || 'Unavngivet')}</a>
                ...
                <td>
                    <div class="flex-col-start">
                        <p class="mb-0 font-13">${escapeHtml(org.cvr || '-')}</p>
                        <p class="mb-0 font-12 color-gray">${escapeHtml(org.company_name || '-')}</p>
                    </div>
                </td>
                <td><p class="mb-0 font-13">${escapeHtml(org.primary_email || '-')}</p></td>
                <td><span class="${status.class} font-11">${status.label}</span></td>
                <td><p class="mb-0 font-13">${formatDate(org.created_at)}</p></td>
            </tr>
        `;
    }
    ...
    function renderPagination() {
        ...
        $pagination.find('a[data-page]').on('click', function(e) {
            ...
            if (page !== currentPage && !isLoading) {
                ...
                fetchOrganisations();
                $('html, body').animate({
                    scrollTop: $('#organisations-table').offset().top - 100
                }, 200);
            }
        });
    }
...
$(document).ready(function() {
    AdminOrganisationsPagination.init();
});
