$(function() {
  ...
  if($('#chartjsDoughnut').length) {
    new Chart($('#chartjsDoughnut'), {
      type: 'doughnut',
      data: {
        labels: ["USA %", "Denmark %", "Germany %","China %","UK %"],
        datasets: [
          {
            label: "Countries percentages",
            backgroundColor: ["#7ee5e5","#f77eb9","#4d8af0","#FF746E","#1b8dbf"],
            data: [59,21,8,7,5]
          }
        ]
      }
  ...
  if($('#chartjsPie').length) {
    new Chart($('#chartjsPie'), {
      type: 'pie',
      data: {
        labels: ["Influencers %", "Real people %","Other %"],
        datasets: [{
          label: "Audience type percentages",
          backgroundColor: ["#1b8dbf","#FF746E","#8F8FF0"],
          data: [9,61,30]
        }]
      }
    });
